...
const TARGET_ENTRY_IDS = ["13372", "13373", "13374"];
const TARGET_ENTRY_SET = new Set(TARGET_ENTRY_IDS);
...
async function captureTransport(
  page: Parameters<Parameters<typeof withBrowserless>[0]>[0]["page"],
  ...
  for (let attempt = 0; attempt < 10 && !transport; attempt += 1) await page.waitForTimeout(600);
...
async function readOrderItems(orderIds: string[]): Promise<OrderItem[]> {
  const rows = await requestSupabase<StagingItemRow[]>("koper_staging_records", {
    query: new URLSearchParams({
      ...
      koper_parent_id: `in.(${orderIds.join(",")})`,
      order: "koper_id.asc",
      limit: "1000",
    }),
  });
...
function resolveItem(
  ...
  const invoiceId = bestInvoice.value?.invoiceId ?? candidateInvoiceIds[0] ?? null;
  const invoice = invoiceId ? invoicesById.get(invoiceId) : undefined;
  const purchaseOrderIds = invoice && Array.isArray(invoice.purchaseOrders)
    ? invoice.purchaseOrders.flatMap((value) => {
        const id = identifier(objectValue(value)?.purchaseOrderId);
        return id ? [id] : [];
      })
    : [];
  const purchaseOrderId = purchaseOrderIds.length === 1 ? purchaseOrderIds[0]! : null;
  const orderCandidates = purchaseOrderId
    ? orderItems.filter((item) => item.parentOrderId === purchaseOrderId)
    : [];
...
const result = await withBrowserless(async ({ page }) => {
  if (process.env.KOPER_STOCK_ENTRY_PILOT_STAGING_WRITE_ENABLED !== "true") {
    throw new Error("Koper stock entry pilot staging write is not explicitly enabled");
  }
  ...
  const headers = new Map<string, EntryHeader>();
  for (let offset = 0; offset < 5_000 && headers.size < TARGET_ENTRY_IDS.length; offset += 1_000) {
    ...
    for (const value of entries) {
      ...
      if (parsed && TARGET_ENTRY_SET.has(parsed.stockMovementId)) headers.set(parsed.stockMovementId, parsed);
    }
    if (entries.length < 1_000) break;
  }
  const missingHeaders = TARGET_ENTRY_IDS.filter((id) => !headers.has(id));
  if (missingHeaders.length > 0) throw new Error(`Pilot stock entries not found: ${missingHeaders.join(",")}`);

  const detailsById = new Map<string, UnknownRecord>();
  const entryItems: EntryItem[] = [];
  for (const entryId of TARGET_ENTRY_IDS) {
    const response = await page.request.get(
      `https://api.koper.com.br/stock/v1/product_entry?entryId=${entryId}&pending=false`,
      { headers: stockTransport.headers, timeout: 12_000 },
    );
    const payload = await readJson(response, `Koper stock entry ${entryId}`);
    detailsById.set(entryId, payload);
    entryItems.push(...parseEntryItems(entryId, payload));
  }

  const invoiceIds = [...new Set(entryItems.flatMap((item) => item.invoiceIds))];
  if (invoiceIds.length === 0) throw new Error("Pilot entries have no XML invoices");
  ...
  for (const invoiceId of invoiceIds) {
    ...
    const payload = await readJson(response, `Koper XML invoice ${invoiceId}`);
  ...
  const unresolved = entryItems.filter((item) => !resolutions.get(item.productMovementId)?.orderItem);
  if (unresolved.length > 0) {
    throw new Error(`Pilot receipt items unresolved: ${unresolved.map((item) => item.productMovementId).join(",")}`);
  }

  const seenAt = new Date();
  const entryRecords = TARGET_ENTRY_IDS.map((entryId) => {
    const header = headers.get(entryId)!;
    const detail = detailsById.get(entryId) ?? {};
    const relatedResolutions = entryItems
      .filter((item) => item.stockMovementId === entryId)
      .map((item) => resolutions.get(item.productMovementId)!)
      .filter(Boolean);
    return createKoperStagingRecord({
      ...
      sanitizedPayload: {
        ...
        invoiceIds: [...new Set(relatedResolutions.flatMap((resolution) => resolution.invoiceId ? [resolution.invoiceId] : []))],
        purchaseOrderIds: [...new Set(relatedResolutions.flatMap((resolution) => resolution.purchaseOrderId ? [resolution.purchaseOrderId] : []))],
        productMovementIds: entryItems
          .filter((item) => item.stockMovementId === entryId)
          .map((item) => item.productMovementId),
      },
  ...
  const invoiceRecords = invoiceIds.map((invoiceId) => {
    const payload = invoicesById.get(invoiceId) ?? {};
    ...
    return createKoperStagingRecord({
      ...
      sanitizedPayload: {
        ...
        productCount: invoiceProductsById.get(invoiceId)?.length ?? 0,
      },
  ...
  const invoiceProductRecords = invoiceIds.flatMap((invoiceId) =>
    (invoiceProductsById.get(invoiceId) ?? []).map((product, index) =>
  ...
  const verify = async (entity: string, expectedIds: string[]): Promise<number> => {
    const rows = await requestSupabase<Array<{ koper_id: string }>>("koper_staging_records", {
      query: new URLSearchParams({
        ...
        source: "eq.koper",
        entity: `eq.${entity}`,
        koper_id: `in.(${expectedIds.map((id) => `\"${id}\"`).join(",")})`,
        sync_state: "eq.present",
        limit: "1000",
      }),
    });
  ...
  const verification = {
    stockEntries: await verify("stock_entry", entryRecords.map((record) => record.koper_id)),
    stockEntryItems: await verify("stock_entry_item", entryItemRecords.map((record) => record.koper_id)),
    xmlInvoices: await verify("xml_invoice", invoiceRecords.map((record) => record.koper_id)),
    xmlInvoiceProducts: await verify("xml_invoice_product", invoiceProductRecords.map((record) => record.koper_id)),
  };
  if (
  ...
  ) throw new Error(`Pilot staging verification failed: ${JSON.stringify(verification)}`);

  return {
    ok: true,
    flowSelected: true,
    blockedWrites,
    targetEntryIds: TARGET_ENTRY_IDS,
    entryItems: entryItems.length,
    invoiceIds,
    invoiceProducts: invoiceProductRecords.length,
    purchaseOrderIds: orderIds,
    matchedOrderItems: entryItems.length - unresolved.length,
    matchMethods: Object.fromEntries(
      [...resolutions.values()].reduce((map, resolution) => {
...
console.log("KOPER_STOCK_ENTRY_PILOT_STAGING_RESULT", JSON.stringify(result));
