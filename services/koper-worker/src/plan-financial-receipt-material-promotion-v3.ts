...
console.log("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V3_START", JSON.stringify({ readOnly: true }));
...
try {
  ...
  console.log("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V3_SOURCE", JSON.stringify({
  ...
  for (let offset = 0, batchIndex = 0; offset < allReceiptIds.length; offset += batchSize, batchIndex += 1) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        console.log("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V3_BATCH", JSON.stringify({
          batchIndex,
          attempt,
        ...
        console.error("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V3_RETRY", JSON.stringify({
          batchIndex,
          attempt,
          message: error instanceof Error ? error.message.slice(0, 500) : "unknown",
        }));
        if (attempt < maxAttempts) await sleep(attempt * 8_000);
      }
    }
    if (!completed) failedBatches += 1;
  }
  ...
  console.log("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V3_DONE", JSON.stringify({
    ok: failedBatches === 0,
    readOnly: true,
    result: {
  ...
  console.error("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V3_FAILED", JSON.stringify({
