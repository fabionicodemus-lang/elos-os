...
try {
  const result = await withBrowserless(async ({ page }) => {
    ...
    const variants = [25, 50, 100, 200, 500];
    const probes: Json[] = [];
    for (const limit of variants) {
      const u = new URL(base);
      u.searchParams.set("allBills", "yes");
      u.searchParams.set("initialDate", "");
      u.searchParams.set("finalDate", "");
      ...
      probes.push({
        ...
        totalNotPaid: body?.totalNotPaid ?? null,
        returned: Array.isArray(body?.bills) ? body.bills.length : null,
        keys: body ? Object.keys(body).sort() : [],
      });
    }
    return { ok: true, readOnly: true, blockedWrites, headerNamesReused: Object.keys(headers).sort(), probes };
  }, { sessionTimeoutMs: 58_000 });
  console.log("KOPER_PAYABLE_AUTH_QUERY_PROBE", JSON.stringify(result));
} catch (error: unknown) {
  console.error("KOPER_PAYABLE_AUTH_QUERY_PROBE_FAILED", JSON.stringify({ message: error instanceof Error ? error.message.slice(0, 1000) : "unknown" }));
}
