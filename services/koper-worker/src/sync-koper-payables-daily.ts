...
try{
 ...
 if(process.argv.includes("--report")){
  ...
  console.log("KOPER_DAILY_SYNC_EXCEPTIONS",JSON.stringify(exceptions.map(item=>{const bill=billById.get(item.billId),detail=detailByBill.get(item.billId),posted=byPayable.get(`koper_bill:${item.billId}`),invoice=detail?.invoiceId?invoiceBySource.get(`KOPER-NFE-${detail.invoiceId}`):undefined;return{...item,billToPayId:bill?.billToPayId??null,dueDateKoper:bill?.dueDate??null,paidKoper:bill?.isPaid??null,invoiceNumber:bill?.invoiceNumber??null,receiptNumber:bill?.receiptNumber??null,originName:bill?.originName??null,supplierNameKoper:detail?.supplierName??null,supplierIdKoper:detail?.supplierId??null,costCenterIdKoper:detail?.costCenterId??null,invoiceIdKoper:detail?.invoiceId??null,projectIdElos:posted?.project_id??invoice?.project_id??null,supplierIdElos:posted?.supplier_id??invoice?.supplier_id??null,amountElos:posted?.amount??null,dueDateElos:posted?.due_date??null,statusElos:posted?.status??null};})));
 }
 if(write){
