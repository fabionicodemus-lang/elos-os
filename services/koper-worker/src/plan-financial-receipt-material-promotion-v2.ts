...
type ExclusionReason =
  ...
  | "missingEntry"
  | "missingReceiptData"
  | "missingReceiptProduct"
  | "ambiguousReceiptProduct"
  | "missingOrderAllocation"
  | "allocationSplit"
  | "missingNativeOrder"
  | "missingNativeOrderItem"
  | "nativeAllocationSplit"
  | "invalidQuantity"
  | "receiptQuantityMismatch"
  | "allocationQuantityMismatch";

type ExclusionCounters = Record<ExclusionReason, number>;
...
const baseSourceId = (value: string | null): string | null =>
  identifier(value)?.split(":")[0] ?? null;
const approximatelyEqual = (left: number, right: number): boolean =>
  Math.abs(left - right) <= Math.max(0.000001, Math.abs(right) * 0.000001);

function increment(counters: ExclusionCounters, reason: ExclusionReason): void {
...
function financialProducts(receiptId: string, payload: Json): FinancialProduct[] {
  ...
  return products.map((raw): FinancialProduct => {
    ...
    return {
      ...
      orders: rawOrders.map((rawOrder): FinancialAllocation => {
...
async function fetchReceiptBatch(receiptIdsToFetch: string[]): Promise<{
  ...
  return withBrowserless(async ({ page }) => {
    ...
    for (let attempt = 0; attempt < 10 && !headers; attempt += 1) {
    ...
    for (let index = 0; index < receiptIdsToFetch.length; index += concurrency) {
      const group = receiptIdsToFetch.slice(index, index + concurrency);
      const resolved = await Promise.all(group.map(async (receiptId): Promise<ReceiptResult> => {
        try {
          ...
          return { receiptId, products: financialProducts(receiptId, payload) };
        } catch (error: unknown) {
          return {
            receiptId,
            products: [],
...
console.log("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V2_START", JSON.stringify({ readOnly: true }));

try {
  console.log("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V2_STAGE", JSON.stringify({ stage: "read_database" }));
  const [entries, items, orders, orderItems, existingReceipts] = await Promise.all([
    ...
    readAll<NativeOrderItem>("procurement_purchase_order_items", {
      select: "id,order_id,source_id,input_id,input_code,input_name",
      company_id: `eq.${env.BOSSA_COMPANY_ID}`,
      source_system: "eq.koper",
  ...
  for (const entryId of programmedEntries) {
    ...
    if (entryItems.some((item) =>
      identifiers(objectValue(item.payload).candidatePurchaseOrderIds).length > 0
    )) continue;
  ...
  const allReceiptIds = unique([...candidateEntries.values()].flat())
  ...
  console.log("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V2_SOURCE", JSON.stringify({
  ...
  for (let offset = 0, batchIndex = 0; offset < allReceiptIds.length; offset += batchSize, batchIndex += 1) {
    ...
    try {
      ...
      for (const result of batch.results) {
        if (!result.error) productsByReceipt.set(result.receiptId, result.products);
      }
      console.log("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V2_BATCH", JSON.stringify({
        batchIndex,
      ...
      failedBatches += 1;
      console.error("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V2_BATCH_FAILED", JSON.stringify({
        batchIndex,
  ...
  const orderBySource = new Map(
    orders.flatMap((order) => order.source_id ? [[order.source_id, order] as const] : []),
  );
  ...
  for (const [entryId, linkedReceiptIds] of candidateEntries) {
    if (alreadyPromoted.has(entryId)) {
      increment(excluded, "alreadyPromoted");
      continue;
    }
    if (!entryById.has(entryId)) {
      increment(excluded, "missingEntry");
      continue;
    }

    const entryItems = itemsByEntry.get(entryId) ?? [];
    const entryPlans: ItemPlan[] = [];
    let entryReason: ExclusionReason | null = null;

    for (const item of entryItems) {
      const payload = objectValue(item.payload);
      const itemReceiptIds = receiptIds(payload)
        .filter((receiptId) => linkedReceiptIds.includes(receiptId));
      if (
        !itemReceiptIds.length
        || itemReceiptIds.some((receiptId) => !productsByReceipt.has(receiptId))
      ) {
      ...
      const financialProduct = best[0]!.product;
      const allocations = financialProduct.orders
        .filter((allocation) => allocation.orderId && allocation.orderProductId);
      ...
      const nativeItems = orderItemsByKey.get(
        `${order.id}:${allocation.orderProductId}`,
      ) ?? [];
      ...
      const allocationQuantity = allocation.productAmount
        ?? allocation.amountReceived
        ?? allocation.amount;
      if (
        sourceQuantity === null || sourceQuantity <= 0
    ...
    if (entryPlans.length !== entryItems.length) {
      increment(excluded, "missingReceiptProduct");
      continue;
    }
  ...
  console.log("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V2_DONE", JSON.stringify({
    ok: failedBatches === 0,
    readOnly: true,
    result: {
    ...
    safeEntryExamples: safeEntries.slice(0, 15),
  }));
} catch (error: unknown) {
  console.error("KOPER_FINANCIAL_RECEIPT_PROMOTION_PLAN_V2_FAILED", JSON.stringify({
