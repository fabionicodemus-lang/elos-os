...
export async function collectFlowStagingSample(): Promise<KoperStagingRecord[]> {
...
export async function previewFlowStaging(): Promise<{
  ...
  const sample = await collectFlowStagingSample();
  const records = sample.map((staging) => ({
