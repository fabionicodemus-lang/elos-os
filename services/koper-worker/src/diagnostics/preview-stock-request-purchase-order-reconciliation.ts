...
export async function previewStockRequestPurchaseOrderReconciliation() {
  const [stockItems, purchaseSourceItems, services, requests, requestItems, purchaseItems] = await Promise.all([
    ...
    readAll<PurchaseItemRow>("procurement_purchase_order_items", {
      select: "id,source_id,request_id,request_item_id,input_id,cost_center_service_id,cost_center_code,notes",
      company_id: `eq.${env.BOSSA_COMPANY_ID}`,
      source_system: "eq.koper",
      order: "id.asc",
    }),
  ...
  const stockByProductRequest = new Map(stockItems.map((row) => [row.koper_id, row]));
  ...
  for (const row of stockItems) {
    ...
    const inputSourceId = identifier(payload.inputId);
    const requestSourceId = row.koper_parent_id ?? identifier(payload.requestId);
    ...
    if (links.length === 0) {
      ...
      if (mismatch && allocationIds.length > 1) {
        ...
        for (const link of effectiveLinks) {
          ...
          if (!resolution) throw new Error(`Official service missing for ${row.koper_id}`);
    ...
    for (const link of splits) {
      ...
      if (currentPurchase.cost_center_service_id === chosen.elosServiceId) {
        ...
        if (repointSamples.length < 30) repointSamples.push({
          ...
          currentRequestNumber: currentPurchase.request_id ? requestById.get(currentPurchase.request_id)?.request_number ?? null : null,
          currentServiceCode: currentPurchase.cost_center_code,
          targetServiceCode: chosen.serviceCode,
          currentProductRequestRows: (currentProductMap.get(productRequestId) ?? []).map((row) => ({ id: row.id, serviceCode: row.cost_center_code })),
        });
      }
    }
  }
  ...
  const staleRequestItemIds = new Set(requestItems
    ...
    .map((row) => row.id));
  const purchaseRowsReferencingStale = purchaseItems.filter((row) => row.request_item_id && staleRequestItemIds.has(row.request_item_id));
  ...
  return {
    ...
    staleRequestItemsBlockedByPurchaseRows: new Set(purchaseRowsReferencingStale.map((row) => row.request_item_id)).size,
    ambiguousSamples,
    repointSamples,
  };
}
