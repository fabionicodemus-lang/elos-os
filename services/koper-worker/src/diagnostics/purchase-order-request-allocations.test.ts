...
test("keeps a single candidate as a direct request item link", () => {
  ...
  if (result.status === "direct") assert.deepEqual(result.allocations.map((row) => [row.item.id, row.quantity]), [["a", 8]]);
});
...
test("uses a unique purchase cost center when it exists", () => {
  ...
  assert.equal(result.status, "direct");
  if (result.status === "direct") assert.equal(result.item.id, "b");
});
...
test("uses product-specific evidence when a v2 request row aggregates multiple productRequestIds", () => {
  const candidates: Candidate[] = [
    {
      id: "a", request_id: "req", cost_center_service_id: "svc-a", requested_quantity: 120,
    ...
    {
      id: "b", request_id: "req", cost_center_service_id: "svc-b", requested_quantity: 80,
  ...
  assert.equal(result.status, "multi");
  if (result.status === "multi") assert.deepEqual(result.allocations.map((row) => [row.item.id, row.quantity]), [["a", 20], ["b", 30]]);
});

test("sums repeated evidence entries for the same productRequestId", () => {
  const candidates: Candidate[] = [
    {
      id: "a", request_id: "req", cost_center_service_id: "svc-a", requested_quantity: 30,
    ...
    {
      id: "b", request_id: "req", cost_center_service_id: "svc-b", requested_quantity: 70,
  ...
  assert.equal(result.status, "multi");
  if (result.status === "multi") assert.deepEqual(result.allocations.map((row) => [row.item.id, row.quantity]), [["a", 30], ["b", 70]]);
});
...
test("fails closed if candidates belong to different material requests", () => {
  const candidates: Candidate[] = [
