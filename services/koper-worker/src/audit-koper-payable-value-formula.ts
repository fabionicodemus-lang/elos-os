...
try{
 const result=await withBrowserless(async({page})=>{
  ...
  let paidBill=0,paidPayment=0,paidEffective=0,openBill=0,openPayment=0,allBill=0,allPayment=0,allEffective=0;
  let discount=0,interest=0,lateFee=0,otherAccruals=0;
  let paidFormulaA=0,openFormulaA=0,allFormulaA=0;
  let paidRows=0,openRows=0,paymentDiffRows=0;
  const largestDiffs:Array<{billId:unknown,billValue:number,paymentValue:number,discount:number,interest:number,lateFee:number,otherAccruals:number,diff:number}>=[];
  for(const r of bills){
   const bv=c(r.billValue),pv=c(r.paymentValue),d=c(r.discount),i=c(r.interest),l=c(r.lateFee),o=c(r.otherAccruals);
   const formula=bv-d+i+l+o;
   allBill+=bv;allPayment+=pv;discount+=d;interest+=i;lateFee+=l;otherAccruals+=o;allFormulaA+=formula;
   const paid=r.isPaid===true;
   if(paid){paidRows++;paidBill+=bv;paidPayment+=pv;paidFormulaA+=formula;paidEffective+=pv||formula;}else{openRows++;openBill+=bv;openPayment+=pv;openFormulaA+=formula;}
   allEffective+=(paid?(pv||formula):formula);
   if(paid&&pv!==bv){paymentDiffRows++;largestDiffs.push({billId:r.billId??null,billValue:br(bv),paymentValue:br(pv),discount:br(d),interest:br(i),lateFee:br(l),otherAccruals:br(o),diff:br(pv-bv)});}
  }
  largestDiffs.sort((a,b)=>Math.abs(b.diff)-Math.abs(a.diff));
  const totalBills=c(header?.totalBills),totalPaid=c(header?.totalPaid),totalNotPaid=c(header?.totalNotPaid);
  return{ok:true,readOnly:true,blockedWrites,count:bills.length,header:{billsAmount:n(header?.billsAmount),totalBills:br(totalBills),totalPaid:br(totalPaid),totalNotPaid:br(totalNotPaid),totalDiscount:n(header?.totalDiscount),totalInterest:n(header?.totalInterest)},sums:{paidRows,openRows,paidBillValue:br(paidBill),paidPaymentValue:br(paidPayment),openBillValue:br(openBill),openPaymentValue:br(openPayment),allBillValue:br(allBill),allPaymentValue:br(allPayment),discount:br(discount),interest:br(interest),lateFee:br(lateFee),otherAccruals:br(otherAccruals),paidFormulaBillMinusDiscountPlusAccruals:br(paidFormulaA),openFormulaBillMinusDiscountPlusAccruals:br(openFormulaA),allFormulaBillMinusDiscountPlusAccruals:br(allFormulaA),effectivePaidPaymentElseFormula:br(allEffective)},differencesToHeader:{paidBillMinusHeaderPaid:br(paidBill-totalPaid),paidPaymentMinusHeaderPaid:br(paidPayment-totalPaid),paidFormulaMinusHeaderPaid:br(paidFormulaA-totalPaid),openBillMinusHeaderOpen:br(openBill-totalNotPaid),openFormulaMinusHeaderOpen:br(openFormulaA-totalNotPaid),allBillMinusHeaderTotal:br(allBill-totalBills),allFormulaMinusHeaderTotal:br(allFormulaA-totalBills),effectiveMinusHeaderTotal:br(allEffective-totalBills)},paymentDiffRows,largestPaymentDiffs:largestDiffs.slice(0,20)};
