...
try{
 const result=await withBrowserless(async({page})=>{
  ...
  const onResponse=(response:Response)=>{try{const u=new URL(response.url());if(response.request().method()!=="GET"||u.hostname!=="api.koper.com.br")return;const task=(async()=>{const ct=response.headers()["content-type"]??"";const body=ct.includes("json")?await response.json().catch(()=>null):null;calls.push({path:u.pathname,status:response.status(),query:Object.fromEntries([...u.searchParams.entries()].filter(([k])=>!/token|auth|cb/i.test(k))),response:shape(body),evidence:ev(body).slice(0,900)});})();pending.push(task);}catch{}};
  page.on("response",onResponse);
  await page.goto("https://app.koper.com.br/engenharia/medicoes",{waitUntil:"domcontentloaded",timeout:15000}).catch(()=>undefined);
  await page.waitForTimeout(4000);
  const finalButton=page.getByText(/VER FINALIZADAS/i).first();if(await finalButton.count())await finalButton.click().catch(()=>undefined);
  await page.waitForTimeout(5000);
  const row=page.locator("tr",{hasText:/\b2624\b/}).first();const rowText=await row.innerText().catch(()=>"");
  if(await row.count())await row.click().catch(()=>undefined);
  await page.waitForTimeout(9000);
  page.off("response",onResponse);await Promise.allSettled(pending);
  return{ok:true,readOnly:true,blockedWrites,rowText,finalUrl:page.url(),body:(await page.locator("body").innerText().catch(()=>"")).replace(/\s+/g," ").slice(0,7000),calls:[...new Map(calls.map(c=>[`${c.path}|${JSON.stringify(c.query)}`,c])).values()]};
 },{sessionTimeoutMs:58_000});console.log("KOPER_MEASUREMENT_2624_NETWORK",JSON.stringify(result));
}catch(error:unknown){console.error("KOPER_MEASUREMENT_2624_NETWORK_FAILED",JSON.stringify({message:error instanceof Error?error.message.slice(0,1000):"unknown"}));}
