...
- **Exceção REST autorizada pelo Fábio em 2026-07-31:** `POST https://api.koper.com.br/login/change_company` pode ser usado exclusivamente para trocar o contexto da sessão entre empresas previamente confirmadas. O corpo deve ser JSON, conter somente as chaves esperadas `accessToken`, `toEnterpriseId` e `changeCompany`, e `toEnterpriseId` deve corresponder ao `enterpriseId` allowlisted de Bossa, Flow ou Alma; qualquer outro formato ou valor deve ser bloqueado. Nunca registrar o valor de `accessToken`. Após a chamada, confirmar a empresa ativa por `GET /administrative/v1/enterprise`. Nenhum outro `POST` REST está autorizado.
