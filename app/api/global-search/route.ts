...
function project(value: unknown): RelatedProject | null {
  return one(value as RelatedProject | RelatedProject[] | null);
}

function projectLabel(value: unknown) {
  const related = project(value);
  return related ? [related.code, related.name].filter(Boolean).join(" · ") : "";
}
...
function rank(result: SearchResult, term: string) {
...
function destination(href: string, resultProjectId: string | null | undefined, activeProjectId: string | null) {
  if (!resultProjectId || resultProjectId === activeProjectId) return href;
  return `/busca/abrir?${new URLSearchParams({ project_id: resultProjectId, dest: href }).toString()}`;
}
...
export async function GET(request: NextRequest) {
  ...
  add(can("clients.view"), safeTask(async () => {
  ...
  add(can("suppliers.view"), safeTask(async () => {
    ...
    return (data ?? []).map((row) => {
      ...
      return {
        ...
        subtitle: [row.legal_name !== name ? row.legal_name : null, row.tax_id, row.category].filter(Boolean).join(" · ") || "Cadastro de fornecedor",
        meta: [row.city, row.state, statusLabel(row.status)].filter(Boolean).join(" · "),
        href: `/cadastros/fornecedores?q=${encodeURIComponent(name)}`,
      };
    });
  }));

  add(can("projects.view"), safeTask(async () => {
    ...
    return (data ?? []).map((row) => ({
      ...
      meta: statusLabel(row.status), href: `/empreendimentos?q=${encodeURIComponent(row.name)}`,
      projectId: row.id, projectName: row.name,
    }));
  }));

  add(can("projects.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("units")
    ...
    return (data ?? []).map((row) => ({
      ...
      projectId: row.project_id, projectName: project(row.projects)?.name ?? null,
    }));
  }));

  add(can("services.view"), safeTask(async () => {
    const { data, error } = await db.from("engineering_services")
  ...
  add(can("inputs.view"), safeTask(async () => {
    const { data, error } = await db.from("engineering_inputs")
  ...
  add(can("commercial.brokers.view"), safeTask(async () => {
    const { data, error } = await db.from("commercial_brokers")
      .select("id,name,kind,legal_name,tax_id,creci_number,creci_state,agency_name,email,phone,status")
      .eq("company_id", companyId)
  ...
  add(can("commercial.proposals.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("commercial_proposals")
    ...
    return (data ?? []).map((row) => {
      ...
      return {
        ...
        href: `/comercial/propostas/${row.id}`, projectId: row.project_id, projectName: project(row.projects)?.name ?? null,
      };
    });
  }));

  add(can("sales.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("sales")
    ...
    return (data ?? []).map((row) => {
      ...
      return {
        ...
        href: `/comercial/vendas?q=${encodeURIComponent(row.number)}`, projectId: row.project_id, projectName: project(row.projects)?.name ?? null,
      };
    });
  }));

  add(can("execution.contracts.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("execution_service_contracts")
  ...
  add(can("execution.work_orders.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("execution_work_orders")
  ...
  add(can("procurement.orders.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("procurement_purchase_orders")
  ...
  add(can("payables.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("payables")
  ...
  add(can("receivables.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("receivables")
  ...
  add(can("postwork.assistance.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("postwork_assistance_tickets")
  ...
  add(can("postwork.warranties.view") && (scope === "company" || Boolean(scopeProjectId)), safeTask(async () => {
    const { data, error } = await scoped(db.from("postwork_warranty_assets")
  ...
  const documentTask = (
    ...
    return rows.map((row) => {
      ...
      return {
        ...
        href: href(parentId), projectId: row.project_id, projectName: project(row.projects)?.name ?? null,
      };
    }).filter((item: SearchResult) => Boolean(item.href));
  }));

  documentTask(can("execution.contracts.view"), "execution_service_contract_documents", "contract_id", "Documentos de contratos", (id) => `/execucao/contratos-servicos?contract=${encodeURIComponent(id)}`);
  documentTask(can("execution.work_orders.view"), "execution_work_order_documents", "work_order_id", "Documentos de O.S.", (id) => `/execucao/ordens-servico?work_order=${encodeURIComponent(id)}`);
  documentTask(can("procurement.orders.view"), "procurement_purchase_order_documents", "order_id", "Documentos de compras", (id) => `/suprimentos/pedidos-compras?order=${encodeURIComponent(id)}`);
  documentTask(can("postwork.assistance.view"), "postwork_assistance_documents", "ticket_id", "Evidências de assistência", (id) => `/pos-obra/assistencias?ticket=${encodeURIComponent(id)}`);
  documentTask(can("postwork.warranties.view"), "postwork_warranty_documents", "asset_id", "Documentos de garantia", (id) => id ? `/pos-obra/garantias?asset=${encodeURIComponent(id)}` : "/pos-obra/garantias");

  const settled = await Promise.allSettled(tasks.map((task) => task()));
  const results = settled
    .flatMap((item) => item.status === "fulfilled" ? item.value : [])
    .map((item) => ({ ...item, href: destination(item.href, item.projectId, projectId), score: rank(item, term) }))
    .sort((a, b) => (b.score ?? 0) - (a.score ?? 0) || a.group.localeCompare(b.group, "pt-BR") || a.title.localeCompare(b.title, "pt-BR"))
    .slice(0, TOTAL_LIMIT)
    .map(({ score: _score, ...item }) => item);

  return NextResponse.json({ results, query: term, scope, activeProjectId: projectId, total: results.length });
}
