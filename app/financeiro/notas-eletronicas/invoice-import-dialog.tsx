...
type MappingSource = "remembered" | "suggested" | "order" | "receipt" | "manual" | "";
type Mapping = {
  line_number: number;
  input_id: string;
...
function productKey(item: ParsedNfe["items"][number]) {
...
function blankMappings(parsed: ParsedNfe, inputs: InvoiceInputOption[], supplierId = "", remembered: RememberedMapping[] = []): Mapping[] {
  const activeInputIds = new Set(inputs.map((input) => input.id));
  const rememberedMap = new Map(remembered.filter((row) => row.supplier_id === supplierId && activeInputIds.has(row.input_id)).map((row) => [row.product_key, row.input_id]));
  return parsed.items.map((item) => {
    const rememberedInputId = rememberedMap.get(productKey(item)) ?? "";
    const suggested = rememberedInputId ? null : bestInput(item, inputs);
    return {
      line_number: item.lineNumber,
      input_id: rememberedInputId || suggested?.id || "",
      order_item_id: "",
      receipt_item_id: "",
      cost_center_service_id: "",
      notes: "",
      match_source: rememberedInputId ? "remembered" : suggested ? "suggested" : "",
    };
  });
}

function InvoiceInputPicker({ value, inputs, onChange }: { value: string; inputs: InvoiceInputOption[]; onChange: (inputId: string) => void }) {
  const buttonRef = useRef<HTMLButtonElement>(null);
  const searchRef = useRef<HTMLInputElement>(null);
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [position, setPosition] = useState({ top: 0, left: 0, width: 420 });
  const selected = inputs.find((input) => input.id === value) ?? null;
  const normalizedQuery = normalizeMatchText(query);
  const filtered = useMemo(() => inputs.filter((input) => {
    if (!normalizedQuery) return true;
    return normalizeMatchText(`${input.code} ${input.description} ${input.unit} ${input.category}`).includes(normalizedQuery);
  }).slice(0, 150), [inputs, normalizedQuery]);
  ...
  useEffect(() => {
    ...
    const updatePosition = () => {
      ...
      if (!rect) return;
      const desiredWidth = Math.min(Math.max(rect.width, 430), window.innerWidth - 16);
      const left = Math.max(8, Math.min(rect.left, window.innerWidth - desiredWidth - 8));
      const popupHeight = 330;
      const below = window.innerHeight - rect.bottom;
      const top = below >= popupHeight || below >= rect.top ? rect.bottom + 4 : Math.max(8, rect.top - popupHeight - 4);
    ...
    const closeOutside = (event: MouseEvent) => {
      ...
      if (target instanceof Element && target.closest(".xml-input-picker-popover")) return;
      setOpen(false);
    };
  ...
  const popover = open && typeof document !== "undefined" ? createPortal(
    <div className="xml-input-picker-popover" style={position}>
      <div className="xml-input-picker-search"><span>⌕</span><input ref={searchRef} value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por código, descrição, unidade ou categoria" /></div>
      <div className="xml-input-picker-options">
        <button type="button" className={!value ? "selected" : ""} onClick={() => { onChange(""); setOpen(false); }}>Selecione o insumo</button>
        {filtered.map((input) => <button key={input.id} type="button" className={input.id === value ? "selected" : ""} onClick={() => { onChange(input.id); setOpen(false); setQuery(""); }}>
          <strong>{input.code}</strong><span>{input.description}</span><small>{input.unit} · {input.category}</small>
        </button>)}
        {!filtered.length ? <div className="xml-input-picker-empty">Nenhum insumo encontrado.</div> : null}
      </div>
      {filtered.length === 150 ? <small className="xml-input-picker-limit">Mostrando os primeiros 150 resultados. Digite mais detalhes para filtrar.</small> : null}
    </div>,
    document.body,
  ) : null;

  return <>
    <button ref={buttonRef} type="button" className={`xml-input-picker-button ${selected ? "selected" : ""}`} onClick={() => setOpen((current) => !current)} title={selected ? `${selected.code} · ${selected.description}` : "Selecionar insumo"}>
      {selected ? <><strong>{selected.code}</strong><span>{selected.description}</span><small>{selected.unit}</small></> : <span>Selecione o insumo</span>}
      <b>⌄</b>
    </button>
    {popover}
  </>;
...
async function loadRememberedMappings(supplierId: string): Promise<RememberedMapping[]> {
  if (!supplierId) return [];
  try {
    const response = await fetch(`/api/financeiro/notas-eletronicas/mapeamentos?supplier_id=${encodeURIComponent(supplierId)}`, { cache: "no-store" });
    if (!response.ok) return [];
    const payload = await response.json() as { mappings?: RememberedMapping[] };
...
export function InvoiceImportDialog({ suppliers, inputs, orders, orderItems, receipts, receiptItems, legalEntity }: {
  ...
  function chooseOrder(nextOrderId: string, sourceParsed = parsed, sourceMappings = mappings) {
    setOrderId(nextOrderId); setReceiptId("");
    ...
    setMappings(sourceParsed.items.map((xmlItem, index) => {
      ...
      const candidate = available.find((item) => current.input_id && item.input_id === current.input_id)
        ...
        ?? available.find((item) => { const itemText = normalizeMatchText(item.input_name); return description.length > 10 && (itemText.includes(description) || description.includes(itemText)); });
      return candidate ? { ...current, input_id: candidate.input_id, order_item_id: candidate.id, receipt_item_id: "", cost_center_service_id: candidate.cost_center_service_id ?? "", match_source: "order" } : { ...current, order_item_id: "", receipt_item_id: "" };
    }));
  }
  ...
  function chooseReceipt(nextReceiptId: string) {
    ...
    if (receipt && receipt.order_id !== orderId) setOrderId(receipt.order_id);
    if (!parsed) return;
    ...
    setMappings((current) => parsed.items.map((xmlItem, index) => {
      ...
      return candidate ? { ...mapping, input_id: candidate.input_id, order_item_id: candidate.order_item_id, receipt_item_id: candidate.id, cost_center_service_id: candidate.cost_center_service_id ?? "", match_source: "receipt" } : mapping;
    }));
  }
      ...
      <form action={importElectronicInvoice} encType="multipart/form-data">
        ...
        <div className="xml-dialog-body">
          <label className={`xml-dropzone ${parsed ? "loaded" : ""}`} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const file = event.dataTransfer.files[0]; if (fileRef.current && file) { const transfer = new DataTransfer(); transfer.items.add(file); fileRef.current.files = transfer.files; void parseFile(file); } }}>
            ...
            <strong>{parsed ? `✓ ${fileName}` : "Arraste o XML da NF-e aqui"}</strong><span>{parsed ? "Arquivo interpretado. Revise os vínculos abaixo." : "ou clique para selecionar o arquivo"}</span><small>Limite de 5 MB · somente XML de NF-e/NFC-e</small>
          </label>
          ...
          {parsed ? <>
            ...
            <section className="xml-link-panel"><div className="xml-section-title"><div><span>Vínculos de origem</span><h3>Fornecedor, pedido e recebimento</h3></div></div><div className="xml-link-grid">
              ...
              <div className="xml-items-list">
                {parsed.items.map((item, index) => {
                  ...
                  return <article key={item.lineNumber} className={`xml-item-row ${mapping?.input_id ? "mapped" : "pending"}`}>
                    <div className="xml-item-product"><div><span>Item {item.lineNumber} · {item.supplierCode || "sem código"}</span>{mapping?.match_source === "remembered" ? <em>✓ associação lembrada</em> : mapping?.match_source === "suggested" ? <em className="suggested">sugestão automática</em> : null}</div><strong>{item.description}</strong><small>{item.quantity} {item.unit} × {money(item.unitPrice)} · NCM {item.ncm || "—"} · CFOP {item.cfop || "—"}</small></div>
                    <div className="xml-item-controls">
                      <div className="xml-input-field"><span>Insumo Elos</span><InvoiceInputPicker value={mapping?.input_id ?? ""} inputs={inputs} onChange={(inputId) => updateMapping(index, { input_id: inputId, match_source: inputId ? "manual" : "" })} /></div>
                      <label>Item do pedido<select value={mapping?.order_item_id ?? ""} disabled={!orderId} onChange={(event) => { const selected = selectedOrderItems.find((row) => row.id === event.target.value); updateMapping(index, { order_item_id: event.target.value, input_id: selected?.input_id ?? mapping?.input_id ?? "", cost_center_service_id: selected?.cost_center_service_id ?? "", match_source: selected ? "order" : mapping?.match_source ?? "" }); }}><option value="">Sem vínculo</option>{selectedOrderItems.map((orderItem) => <option key={orderItem.id} value={orderItem.id}>{orderItem.input_code} · {orderItem.input_name} · {orderItem.ordered_quantity} {orderItem.unit_snapshot}</option>)}</select></label>
                      <label>Item recebido<select value={mapping?.receipt_item_id ?? ""} disabled={!receiptId} onChange={(event) => { const selected = selectedReceiptItems.find((row) => row.id === event.target.value); updateMapping(index, { receipt_item_id: event.target.value, order_item_id: selected?.order_item_id ?? mapping?.order_item_id ?? "", input_id: selected?.input_id ?? mapping?.input_id ?? "", cost_center_service_id: selected?.cost_center_service_id ?? "", match_source: selected ? "receipt" : mapping?.match_source ?? "" }); }}><option value="">Sem vínculo</option>{selectedReceiptItems.map((receiptItem) => <option key={receiptItem.id} value={receiptItem.id}>{receiptItem.input_code} · {receiptItem.input_name} · {receiptItem.accepted_quantity} {receiptItem.unit_snapshot}</option>)}</select></label>
                    </div>
                  </article>;
                })}
              </div>
