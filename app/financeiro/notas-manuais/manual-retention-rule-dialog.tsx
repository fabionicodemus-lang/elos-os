...
const defaults: Record<string, Omit<ManualRetentionRuleOption, "id" | "project_id" | "authority_supplier_id">> = {
  ...
  iss: { retention_key: "iss", code: "ISS-RET", name: "ISS Retido", fiscal_class: "Tributo municipal retido", payment_method: "Guia municipal", document_prefix: "ISS", due_trigger: "issue_date", due_rule: "manual", due_day: null, due_month_offset: 1, due_days_after_event: 0, business_day_adjustment: "none" },
  irrf: { retention_key: "irrf", code: "IRRF-RET", name: "IRRF Retido", fiscal_class: "Imposto de renda retido", payment_method: "DARF", document_prefix: "IRRF", due_trigger: "issue_date", due_rule: "fixed_day", due_day: 20, due_month_offset: 1, due_days_after_event: 0, business_day_adjustment: "previous_weekday" },
  pis: { retention_key: "pis", code: "PIS-RET", name: "PIS Retido", fiscal_class: "Contribuição federal retida", payment_method: "DARF", document_prefix: "PIS", due_trigger: "first_installment_due_date", due_rule: "fixed_day", due_day: 20, due_month_offset: 1, due_days_after_event: 0, business_day_adjustment: "previous_weekday" },
  cofins: { retention_key: "cofins", code: "COFINS-RET", name: "COFINS Retida", fiscal_class: "Contribuição federal retida", payment_method: "DARF", document_prefix: "COFINS", due_trigger: "first_installment_due_date", due_rule: "fixed_day", due_day: 20, due_month_offset: 1, due_days_after_event: 0, business_day_adjustment: "previous_weekday" },
  csll: { retention_key: "csll", code: "CSLL-RET", name: "CSLL Retida", fiscal_class: "Contribuição federal retida", payment_method: "DARF", document_prefix: "CSLL", due_trigger: "first_installment_due_date", due_rule: "fixed_day", due_day: 20, due_month_offset: 1, due_days_after_event: 0, business_day_adjustment: "previous_weekday" },
  other: { retention_key: "other", code: "OUTRA-RET", name: "Outro Imposto Retido", fiscal_class: "Outro tributo retido", payment_method: "Guia", document_prefix: "RET", due_trigger: "issue_date", due_rule: "manual", due_day: null, due_month_offset: 1, due_days_after_event: 0, business_day_adjustment: "none" },
};
...
export function ManualRetentionRuleDialog({
  ...
  const missing = useMemo(() => Object.keys(retentionLabels).find((key) => !rules.some((rule) => rule.retention_key === key && rule.due_rule !== "manual")), [rules]);
  ...
  const configured = rules.filter((rule) => rule.due_rule !== "manual" && rule.authority_supplier_id).length;
      ...
      <form action={saveManualRetentionRule}>
        ...
        <header>
          <div>
            ...
            <p>{projectLabel} · defina o órgão e o vencimento sugerido. A data gerada continuará visível e rastreável na agenda fiscal.</p>
          </div>
        ...
        <div className="tax-dialog-body">
          <div className="tax-form-grid">
            ...
            <label>Data-base
              <select name="due_trigger" value={model.due_trigger} onChange={(event) => setModel((current) => ({ ...current, due_trigger: event.target.value as ManualRetentionRuleOption["due_trigger"] }))}>
            ...
            <label>Regra de vencimento
              <select name="due_rule" value={model.due_rule === "manual" ? "fixed_day" : model.due_rule} onChange={(event) => setModel((current) => ({ ...current, due_rule: event.target.value as ManualRetentionRuleOption["due_rule"] }))}>
                <option value="fixed_day">Dia fixo do mês</option>
                <option value="days_after_event">Quantidade de dias após a data-base</option>
              </select>
            </label>
            {(model.due_rule === "fixed_day" || model.due_rule === "manual") ? <>
              <label>Dia do vencimento<input name="due_day" type="number" min="1" max="31" value={model.due_day ?? 20} onChange={(event) => setModel((current) => ({ ...current, due_rule: "fixed_day", due_day: Number(event.target.value) }))} required /></label>
          ...
          <div className="tax-form-note">
            ...
            <span>O sistema aplica a regra cadastrada e cria a obrigação automaticamente. Feriados bancários e regras excepcionais devem ser conferidos pela área fiscal antes do pagamento.</span>
          </div>
        </div>
        <footer>
          <button type="button" onClick={() => ref.current?.close()}>Cancelar</button>
          <button className="primary" type="submit">Salvar regra de {retentionLabels[retentionKey]}</button>
        </footer>
      </form>
    </dialog>
  </>;
