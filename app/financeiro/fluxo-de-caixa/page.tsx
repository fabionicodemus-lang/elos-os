...
function originLabel(value: string | null, fallback: string) {
  if (!value || value === "elos_os") return fallback;
  if (value.includes("koper")) return "Koper";
  return value;
}
...
export default async function CashFlowPage({
  ...
  const payableEntries: CashEntry[] = payables.map((payable) => {
    ...
    return {
      ...
      paidAt: cleanDate(payable.paid_at) || null,
      amount: Number(paid ? payable.paid_amount ?? payable.amount : payable.amount),
      party: supplier?.trade_name || supplier?.legal_name || "Fornecedor",
      reference: payable.document || payable.notes || "Conta a pagar",
      installment: payable.installment_label || "—",
      account: payable.paid_account_name || "—",
      origin: originLabel(payable.source_system, "Contas a Pagar"),
  ...
  const receivableEntries: CashEntry[] = receivables.map((receivable) => {
    ...
    return {
      ...
      paidAt: cleanDate(receivable.paid_at) || null,
      ...
      party: client?.name || "Cliente",
      reference: `${unit?.code ? `Unidade ${unit.code}` : "Unidade"}${sale?.number ? ` · Venda ${sale.number}` : ""}`,
      ...
      account: receivable.paid_account_name || "—",
      origin: originLabel(receivable.source_system, "Contas a Receber"),
      link: `/comercial/vendas/${receivable.sale_id}`,
    };
  });
  ...
  const entries = allEntries
    ...
    .filter((entry) => {
      ...
      return [entry.party, entry.reference, entry.installment, entry.account, entry.origin]
        .join(" ")
        .toLowerCase()
        .includes(queryText);
    });
  ...
  const bucketMap = new Map<string, Omit<Bucket, "periodBalance" | "accumulatedBalance">>();
  ...
  const buckets: Bucket[] = [...bucketMap.values()]
    ...
    .map((bucket) => {
      ...
      return { ...bucket, periodBalance, accumulatedBalance };
    });

  const maxBarValue = Math.max(
    1,
    ...buckets.flatMap((bucket) => [bucket.received, bucket.receivable, bucket.realized, bucket.payable]),
  );
  ...
  return (
    <AppShell
      ...
      description={`${company.name} · ${context} · previsto x realizado de ${periodDescription}.`}
      actions={
        <>
    ...
    >
      {payablesResult.error || receivablesResult.error ? (
        <div className="auth-message error workspace-message">Não foi possível consolidar o fluxo de caixa. Confira as estruturas de Contas a Pagar e Contas a Receber.</div>
      ) : null}
      ...
      <section className="finance-summary-grid cashflow-summary-grid">
        ...
        <article><span>A receber</span><strong>{money(toReceive)}</strong><small>carteira atualizada em aberto</small></article>
        <article><span>Realizado</span><strong>{money(realizedOut)}</strong><small>saídas efetivamente pagas</small></article>
        <article><span>A realizar</span><strong>{money(toPay)}</strong><small>compromissos em aberto</small></article>
        ...
        <article><span>Saldo projetado</span><strong className={projectedBalance < 0 ? "cashflow-negative" : ""}>{money(projectedBalance)}</strong><small>entradas totais menos saídas totais</small></article>
      </section>

      <section className="registry-toolbar cashflow-toolbar">
        <form className="cashflow-filter" method="get">
          <input name="q" defaultValue={params.q ?? ""} placeholder="Cliente, fornecedor, unidade, documento ou conta" />
      ...
      <section className="cashflow-chart-panel">
        ...
        <div className="cashflow-legend">
          ...
          <span><i className="receivable" />A receber</span>
          <span><i className="realized" />Realizado</span>
          <span><i className="payable" />A realizar</span>
        </div>

        <div className="cashflow-chart-scroll">
          <div className="cashflow-chart" style={{ minWidth: `${Math.max(920, buckets.length * 94)}px` }}>
            {buckets.map((bucket) => (
              <article className="cashflow-period" key={bucket.key}>
                <div className="cashflow-bars" title={`${bucket.label} · saldo ${money(bucket.periodBalance)}`}>
                  <span className="cashflow-bar received" style={{ height: `${Math.max(bucket.received > 0 ? 3 : 0, (bucket.received / maxBarValue) * 100)}%` }} />
                  <span className="cashflow-bar receivable" style={{ height: `${Math.max(bucket.receivable > 0 ? 3 : 0, (bucket.receivable / maxBarValue) * 100)}%` }} />
                  <span className="cashflow-bar realized" style={{ height: `${Math.max(bucket.realized > 0 ? 3 : 0, (bucket.realized / maxBarValue) * 100)}%` }} />
                  <span className="cashflow-bar payable" style={{ height: `${Math.max(bucket.payable > 0 ? 3 : 0, (bucket.payable / maxBarValue) * 100)}%` }} />
                </div>
                <strong>{bucket.label}</strong>
                <small className={bucket.accumulatedBalance < 0 ? "negative" : ""}>{compactMoney(bucket.accumulatedBalance)}</small>
              </article>
            ))}
            {buckets.length === 0 ? <div className="cashflow-empty">Nenhum lançamento encontrado para montar o gráfico.</div> : null}
          </div>
        </div>
        <p className="cashflow-chart-caption">O valor abaixo de cada período representa o saldo acumulado projetado até aquele ponto.</p>
      </section>

      <section className="registry-table-panel cashflow-table-panel">
        <div className="section-heading">
          <div><span>Planilha financeira</span><h2>Todos os lançamentos consolidados</h2></div>
          <p>{entries.length} lançamento(s) no filtro atual.</p>
        </div>
        <div className="registry-table-wrap">
          <table className="registry-table cashflow-table">
            <thead>
              <tr><th>Data</th><th>Movimento</th><th>Situação</th><th>Cliente / fornecedor</th><th>Documento / contrato / unidade</th><th>Parcela</th><th>Vencimento</th><th>Baixa</th><th>Conta bancária</th><th>Origem</th><th>Valor</th><th>Ação</th></tr>
            </thead>
            <tbody>
              {pageEntries.map((entry) => (
                <tr key={entry.id}>
                  <td>{dateBR(entry.effectiveDate)}</td>
                  <td><span className={`cashflow-direction ${entry.direction}`}>{entry.direction === "in" ? "Entrada" : "Saída"}</span></td>
                  <td><span className={`status-badge ${entry.situation === "realized" ? "paid" : "open"}`}>{entry.situation === "realized" ? "Realizado" : "Previsto"}</span></td>
                  <td><strong>{entry.party}</strong></td>
                  <td><span>{entry.reference}</span></td>
                  <td><span>{entry.installment}</span></td>
                  <td>{dateBR(entry.dueDate)}</td>
                  <td>{dateBR(entry.paidAt)}</td>
                  <td><span>{entry.account}</span></td>
                  <td><span>{entry.origin}</span></td>
                  <td><strong className={entry.direction === "out" ? "cashflow-negative" : ""}>{entry.direction === "out" ? "− " : "+ "}{money(entry.amount)}</strong></td>
                  <td><Link className="table-action" href={entry.link}>Abrir</Link></td>
                </tr>
              ))}
              {pageEntries.length === 0 ? <tr><td className="empty-table" colSpan={12}>Nenhum lançamento encontrado.</td></tr> : null}
            </tbody>
          </table>
        </div>
        <div className="registry-pagination">
          ...
          <div>
            {safePage > 1 ? <Link href={buildQuery(filterParams, safePage - 1)}>Anterior</Link> : null}
            {safePage < totalPages ? <Link href={buildQuery(filterParams, safePage + 1)}>Próxima</Link> : null}
          </div>
        </div>
      </section>
    </AppShell>
