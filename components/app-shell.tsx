...
export async function AppShell({ activeGroup, activeItem, eyebrow, title, description, actions, children }: {
  ...
  return <div className="elos-app-shell">
    ...
    <main className="elos-main">
      ...
      <div className="elos-module-content"><div className="elos-page-top"><div><div className="elos-eyebrow">{eyebrow}</div><h1>{title}<span className="elos-module-version">V0.50.0 · sistema integrado</span></h1>{description ? <p>{description}</p> : null}</div>{actions ? <div className="elos-page-actions">{actions}</div> : null}</div>{children}</div>
    </main>
  </div>;
}
