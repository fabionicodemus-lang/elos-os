...
export async function AppShell({ activeGroup, activeItem, eyebrow, title, description, actions, children }: {
  ...
  const groups: ShellNavigationGroup[] = [
    ...
    { key: "procurement", label: "Suprimentos", icon: "🛒", active: activeGroup === "procurement", items: [
      ...
      { label: "Pedidos de Compras", disabled: true }, { label: "Estoque", disabled: true },
    ...
    { key: "finance", label: "Financeiro", icon: "$", active: activeGroup === "finance", items: [
      ...
      { label: "Notas Manuais", disabled: true }, { label: "Notas Eletrônicas · XML", disabled: true }, { label: "Contas Bancárias", disabled: true }, { label: "Impostos", disabled: true },
    ]},
    { key: "commercial", label: "Comercial", icon: "◇", active: activeGroup === "commercial", items: [
      ...
      { label: "Propostas", disabled: true }, { label: "Corretores", disabled: true },
    ...
    { key: "postwork", label: "Pós-Obra", icon: "⌁", active: activeGroup === "postwork", items: [
      { label: "Assistências Técnicas", disabled: true }, { label: "Vistorias e Chamados", disabled: true }, { label: "Garantias", disabled: true },
    ]},
  ];
  ...
  return <div className="elos-app-shell">
    ...
    <main className="elos-main">
      ...
      <div className="elos-module-content"><div className="elos-page-top"><div><div className="elos-eyebrow">{eyebrow}</div><h1>{title}<span className="elos-module-version">V0.45.0 · sistema integrado</span></h1>{description ? <p>{description}</p> : null}</div>{actions ? <div className="elos-page-actions">{actions}</div> : null}</div>{children}</div>
    </main>
  </div>;
}
