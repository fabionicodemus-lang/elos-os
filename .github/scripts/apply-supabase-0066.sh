...
if [[ -n "${GH_TOKEN:-}" && -n "${PR_NUMBER:-}" ]]; then
  gh api "repos/${GITHUB_REPOSITORY}/issues/${PR_NUMBER}/comments" \
    -f body='✅ Migration 0066 aplicada e validada. Controle integrado, reservas, transferências entre obras, avaliação automática de fornecedores e bases dos indicadores estão ativos.' >/dev/null
fi
