...
if [[ -n "${GH_TOKEN:-}" && -n "${PR_NUMBER:-}" ]]; then
  gh api "repos/${GITHUB_REPOSITORY}/issues/${PR_NUMBER}/comments" \
    -f body='✅ Migration 0067 aplicada e validada. Solicitação de serviços, mapa de concorrência, fornecedor vencedor, geração de contrato e medições por etapas estão ativos.' >/dev/null
fi
