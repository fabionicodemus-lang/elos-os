...
report_failure() {
  ...
  sanitize_log
  {
    echo '❌ As migrations 0056 de Papéis e Permissões falharam.'
    echo
    echo '```text'
    tail -n 320 "$SAFE_LOG"
    echo '```'
...
gh pr comment "$PR_NUMBER" --body 'Iniciando as migrations `0056` e `0056a`: papéis personalizados, matriz de permissões, proteções administrativas e auditoria.'
...
psql "$EFFECTIVE_DB_URL" \
  --no-psqlrc \
  --set=ON_ERROR_STOP=1 \
  --file=supabase/migrations/20260729_0056a_system_permissions_hardening.sql \
  >>"$LOG_FILE" 2>&1
...
psql "$EFFECTIVE_DB_URL" \
  ...
  --command="do \$\$ begin
    ...
    if has_table_privilege('authenticated','public.roles','INSERT') or has_table_privilege('authenticated','public.roles','UPDATE') or has_table_privilege('authenticated','public.roles','DELETE') then raise exception 'Alteração direta de papéis ainda permitida'; end if;
    if has_table_privilege('authenticated','public.role_permissions','INSERT') or has_table_privilege('authenticated','public.role_permissions','UPDATE') or has_table_privilege('authenticated','public.role_permissions','DELETE') then raise exception 'Alteração direta da matriz ainda permitida'; end if;
...
sanitize_log
cat "$SAFE_LOG"
gh pr comment "$PR_NUMBER" --body "✅ Migrations 0056 e 0056a aplicadas e validadas. Papéis personalizados, matriz de acessos, proteção do proprietário/administrador, bloqueio de alterações diretas e auditoria foram confirmados. Execução: https://github.com/${GITHUB_REPOSITORY}/actions/runs/${GITHUB_RUN_ID}"
trap - ERR
